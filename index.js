...
export default class ImageView extends Component<PropsType> {
  ...
  componentWillReceiveProps(nextProps: PropsType) {
    const { isVisible, source, imageMinScale } = this.state;
    ...
    if (nextProps.source !== source) {
      const imageNextMinScale = calculateMinScale(
        nextProps.imageWidth,
        nextProps.imageHeight
      );
  ...
  onGestureMove(event: EventType, gestureState: GestureState) {
    ...
    if (scale === imageMinScale && imageHeight * imageMinScale < screenHeight) {
      ...
      this.backgroundOpacity.setValue(
  ...
  onGestureRelease(event: EventType, gestureState: GestureState) {
    const { imageScale, imageWidth, imageHeight, imageMinScale } = this.state;
    ...
    if (scale === imageMinScale && Math.abs(vy) >= CLOSE_SPEED) {
      Animated.timing(this.traslateValue.y, {
      ...
      >
        ...
        {(renderFooter || title) && (
          ...
          >
            ...
            {title &&
              !renderFooter && <Text style={styles.title}>{title}</Text>}
          </Animated.View>
        )}
      </Animated.Modal>
    );
